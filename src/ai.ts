...
- 문장 끝의 불필요한 줄바꿈(개행)은 오류로 보지 않는다.
- 오직 JSON만 출력한다.`;
...
/** True if the only difference is trailing newline/whitespace — not a real spelling error. */
function isTrailingNewlineOnlyChange(original: string, corrected: string): boolean {
  const trim = (s: string) => s.replace(/\s+$/, "");
  return trim(original) === trim(corrected);
}
    ...
    chunks.map(async (slice) => {
      ...
      slice.forEach((c, idx) => {
        ...
        if (
          ...
          !isTrailingNewlineOnlyChange(c.text, raw.corrected)
        ) {
          found.push({
            nodeId: c.nodeId,
            nodeName: c.nodeName,
